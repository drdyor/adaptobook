...
var isOAuthConfigured = Boolean(ENV2.oAuthServerUrl?.trim()) && Boolean(ENV2.appId?.trim()) && Boolean(ENV2.cookieSecret?.trim());
...
async function invokeLLM(params) {
  ...
  const apiKey = useOpenRouter ? ENV2.openRouterApiKey : ENV2.forgeApiKey;
  const payload = {
...
async function setupVite(app, server) {
  ...
  const clientRoot = path2.resolve(projectRoot, "client");
  const serverOptions = {
    middlewareMode: true,
    hmr: { server },
    allowedHosts: true
